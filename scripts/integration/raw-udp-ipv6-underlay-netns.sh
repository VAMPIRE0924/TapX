#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILD_DIR="${ROOT}/build/integration/netns"
CORE_BIN="${BUILD_DIR}/tapx-core"
NS_A="tapx-it-v6-a"
NS_B="tapx-it-v6-b"
VETH_A="tapxv6-a"
VETH_B="tapxv6-b"
TUN_A="tapxv6tun-a"
TUN_B="tapxv6tun-b"
PID_A=""
PID_B=""

cleanup() {
  set +e
  [[ -z "$PID_A" ]] || { kill "$PID_A" >/dev/null 2>&1; wait "$PID_A" >/dev/null 2>&1; }
  [[ -z "$PID_B" ]] || { kill "$PID_B" >/dev/null 2>&1; wait "$PID_B" >/dev/null 2>&1; }
  ip netns delete "$NS_A" >/dev/null 2>&1
  ip netns delete "$NS_B" >/dev/null 2>&1
}

fail_with_logs() {
  echo "raw UDP/IPv6-underlay integration failed" >&2
  for log in "${BUILD_DIR}/tapx-v6-a.log" "${BUILD_DIR}/tapx-v6-b.log"; do
    [[ ! -f "$log" ]] || { echo "== $log ==" >&2; sed -n '1,160p' "$log" >&2; }
  done
  exit 1
}

wait_for_log() {
  local log="$1" text="$2"
  for _ in $(seq 1 100); do
    [[ ! -f "$log" ]] || ! grep -q "$text" "$log" || return 0
    sleep 0.05
  done
  return 1
}

wait_for_link() {
  local ns="$1" name="$2"
  for _ in $(seq 1 100); do
    ip -n "$ns" link show dev "$name" >/dev/null 2>&1 && return 0
    sleep 0.05
  done
  return 1
}

wait_for_ping() {
  local ns="$1"
  shift
  for _ in $(seq 1 30); do
    ip netns exec "$ns" ping -c 1 -W 1 "$@" >/dev/null 2>&1 && return 0
    sleep 0.1
  done
  return 1
}

snmp6_value() {
  local ns="$1" key="$2"
  ip netns exec "$ns" awk -v key="$key" '$1 == key { print $2; exit }' /proc/net/snmp6
}

[[ "$(id -u)" == "0" ]] || { echo "SKIP: test requires root or CAP_NET_ADMIN"; exit 0; }
for tool in go ip ping grep sed awk; do
  command -v "$tool" >/dev/null 2>&1 || { echo "missing required command: $tool" >&2; exit 1; }
done

mkdir -p "$BUILD_DIR"
trap cleanup EXIT
cleanup
rm -f "${BUILD_DIR}/tapx-v6-"{a,b}.{json,log}

(cd "$ROOT" && GOTOOLCHAIN="${GOTOOLCHAIN:-auto}" go build -o "$CORE_BIN" ./cmd/tapx-core)

cat >"${BUILD_DIR}/tapx-v6-a.json" <<JSON
{
  "Devices": [{"ID":"tun-a","Enabled":true,"Type":"tun","IfName":"${TUN_A}","MTU":1500,"LinkAutoOptimize":true}],
  "Routes": [{"ID":"route-a","Enabled":true,"DeviceID":"tun-a"}],
  "Listeners": [{
    "ID":"udp-v6-a","Enabled":true,"BindHost":"fd31:250::1","BindPort":44001,"Transport":"udp",
    "RawUDP":{"PeerMode":"fixed","FixedPeer":"[fd31:250::2]:44001"},"Binding":{"RouteID":"route-a"}
  }]
}
JSON

cat >"${BUILD_DIR}/tapx-v6-b.json" <<JSON
{
  "Devices": [{"ID":"tun-b","Enabled":true,"Type":"tun","IfName":"${TUN_B}","MTU":1500,"LinkAutoOptimize":true}],
  "Routes": [{"ID":"route-b","Enabled":true,"DeviceID":"tun-b"}],
  "Listeners": [{
    "ID":"udp-v6-b","Enabled":true,"BindHost":"fd31:250::2","BindPort":44001,"Transport":"udp",
    "RawUDP":{"PeerMode":"fixed","FixedPeer":"[fd31:250::1]:44001"},"Binding":{"RouteID":"route-b"}
  }]
}
JSON

ip netns add "$NS_A"
ip netns add "$NS_B"
ip link add "$VETH_A" type veth peer name "$VETH_B"
ip link set "$VETH_A" netns "$NS_A"
ip link set "$VETH_B" netns "$NS_B"
ip -n "$NS_A" link set lo up
ip -n "$NS_B" link set lo up
ip -n "$NS_A" addr add fd31:250::1/64 dev "$VETH_A" nodad
ip -n "$NS_B" addr add fd31:250::2/64 dev "$VETH_B" nodad
ip -n "$NS_A" link set "$VETH_A" mtu 1280 up
ip -n "$NS_B" link set "$VETH_B" mtu 1280 up

ip netns exec "$NS_A" "$CORE_BIN" -config "${BUILD_DIR}/tapx-v6-a.json" >"${BUILD_DIR}/tapx-v6-a.log" 2>&1 &
PID_A="$!"
ip netns exec "$NS_B" "$CORE_BIN" -config "${BUILD_DIR}/tapx-v6-b.json" >"${BUILD_DIR}/tapx-v6-b.log" 2>&1 &
PID_B="$!"

wait_for_log "${BUILD_DIR}/tapx-v6-a.log" "runtime started" || fail_with_logs
wait_for_log "${BUILD_DIR}/tapx-v6-b.log" "runtime started" || fail_with_logs
wait_for_link "$NS_A" "$TUN_A" || fail_with_logs
wait_for_link "$NS_B" "$TUN_B" || fail_with_logs

ip -n "$NS_A" addr add 10.89.0.1/30 dev "$TUN_A"
ip -n "$NS_B" addr add 10.89.0.2/30 dev "$TUN_B"
ip -n "$NS_A" link set "$TUN_A" up
ip -n "$NS_B" link set "$TUN_B" up
ip netns exec "$NS_A" ping -6 -c 1 -W 1 fd31:250::2 >/dev/null || fail_with_logs

frag_a_before="$(snmp6_value "$NS_A" Ip6FragCreates)"
frag_b_before="$(snmp6_value "$NS_B" Ip6FragCreates)"
reasm_a_before="$(snmp6_value "$NS_A" Ip6ReasmReqds)"
reasm_b_before="$(snmp6_value "$NS_B" Ip6ReasmReqds)"

wait_for_ping "$NS_A" 10.89.0.2 || fail_with_logs
wait_for_ping "$NS_B" 10.89.0.1 || fail_with_logs
wait_for_ping "$NS_A" -M do -s 1400 10.89.0.2 || fail_with_logs
wait_for_ping "$NS_B" -M do -s 1400 10.89.0.1 || fail_with_logs

[[ "$(snmp6_value "$NS_A" Ip6FragCreates)" == "$frag_a_before" ]] || fail_with_logs
[[ "$(snmp6_value "$NS_B" Ip6FragCreates)" == "$frag_b_before" ]] || fail_with_logs
[[ "$(snmp6_value "$NS_A" Ip6ReasmReqds)" == "$reasm_a_before" ]] || fail_with_logs
[[ "$(snmp6_value "$NS_B" Ip6ReasmReqds)" == "$reasm_b_before" ]] || fail_with_logs

echo "raw UDP/TUN over 1280-byte IPv6 underlay: ok"
