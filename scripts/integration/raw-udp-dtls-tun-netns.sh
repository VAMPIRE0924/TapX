...
{
  "Devices": [
    {"ID": "tun-a", "Enabled": true, "Type": "tun", "IfName": "${TUN_A}", "MTU": 1400}
  ],
  "VKeys": [
...
{
  "Devices": [
    {"ID": "tun-b", "Enabled": true, "Type": "tun", "IfName": "${TUN_B}", "MTU": 1400}
  ],
  "VKeys": [
...
ip netns exec "$NS_A" ping -c 3 -W 1 10.93.0.2 >/dev/null || fail_with_logs
ip netns exec "$NS_B" ping -c 3 -W 1 10.93.0.1 >/dev/null || fail_with_logs
