...
tar -C "$work_dir" -czf "$dist_dir/$linux_name.tar.gz" "$linux_name"
tar -C "$work_dir" -czf "$dist_dir/$openwrt_name.tar.gz" "$openwrt_name"

(
  cd "$dist_dir"
  sha256sum "$linux_name.tar.gz" "$openwrt_name.tar.gz" > SHA256SUMS
)

echo "$dist_dir/$linux_name.tar.gz"
echo "$dist_dir/$openwrt_name.tar.gz"
