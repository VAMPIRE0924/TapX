...
```text
tapx-panel
  Web UI / API / SQLite / login / logs / backup / stats display
        |
        | local control API
        v
tapx-core-go
...
The initial panel backend uses a flexible SQLite object store:
...
login, JSON backup export/restore, and an in-process operation log for
...
never reads SQLite, parses JSON, or calls panel APIs per packet.
...
`ip`/iptables operations, then records applied state for rollback when the pipe
