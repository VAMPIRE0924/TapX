...
probes with DF policy, commits the final random probe on both peers, and then
passes one immutable datagram/MTU/MSS plan to the worker. Raw DTLS runs the same
protocol after its certificate handshake and accounts for the negotiated DTLS
record expansion. An unset switch preserves the original bare Raw UDP path and
does not run probes or enable the `TXS1` segment format.

Raw TCP/TLS uses an outer-socket `TCP_MAXSEG` ceiling derived from the Device
MTU and outer address family, while Linux TCP PMTU handling may reduce it
further. Generated Xray TCP-like transports receive the same kind of MSS
ceiling, mKCP receives a bounded transport MTU, and QUIC/Hysteria keeps native
path-MTU discovery enabled. These values are generated before workers start;
the disabled Device switch adds none of these settings.
...
sharing API/UI: raw clients generate `raw://` import links, supported Xray
clients generate native protocol links, and QR-code/subscription output is not
part of TapX.
