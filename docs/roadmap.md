...
- SQLite-default flexible object store and migrations. (Implemented.)
- Optional PostgreSQL control-plane store, installer selection, and portable
  `.db` backup/restore shared with SQLite. Cross-backend restore is supported;
  no database work enters the packet path. (Implemented.)
...
- Database-backed panel operation log API and UI. (Implemented with a 500-entry
  retention limit, restart recovery, persistent clear, and backup v3 support.)
...
- Client credential and direct-link sharing for Raw TapX and supported Xray
  protocols. (Implemented without QR-code or subscription output.)
...
- Minimal LuCI service bridge. (Initial UCI settings, core/panel service status
  and actions, recent logs, and full-panel launch exist.)
