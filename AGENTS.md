...
- Embedded Xray must use an unmodified official `github.com/xtls/xray-core` release. Do not fork, vendor, patch, or redirect it with a private `replace`; keep all TapX integration in TapX-owned adapter/config/runtime packages and absorb upstream API changes there.
