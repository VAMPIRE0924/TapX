...
function serviceCard(name, section, initPath, running, autostart, checkboxId, ready) {
	function action(command) {
		...
		var serviceCommand = section === 'core' ? 'runtime_' + command : command;
		...
		return persist.then(function() { return run(initPath, [ serviceCommand ]); }).then(function() { window.setTimeout(function() { window.location.reload(); }, 350); }).catch(function(error) {
			if (!enabledChanged) {
